...
void setup() {
 ...
 for (int layer = 0; layer<tot_layers; layer++) {
   ...
   if (layer%2==0) {
     for (float angle = radians(30); angle<=radians(330); angle+=angle_increment) {
      ...
      float rotationCenter = x_center_table;
      
       float x_rotated = rotationCenter + x_1 * cos(phase) - y_1 * sin(phase);
       float y_rotated = y_center_table + x_1 * sin(phase) + y_1 * cos(phase);

       PVector next_point = new PVector(x, y);
       if (layer == 0 && angle==0) {
         // Go to starting position
         gCommand("G1 X" + x_rotated + " Y" + y_rotated);
       } else {
         if (angle == 0 ) {
           gCommand("G0 X" + x_rotated + " Y" + y_rotated);

           println(layer);
         }
           extrusion += (extrude(previous_point, next_point) * extrusion_multiplier);
           gCommand("G1 X" + x_rotated + " Y" + y_rotated + " E" + extrusion);
   ...
   if (layer%2 == 1) {
       for (float angle = radians(210) ; angle<=radians(510); angle+=angle_increment) {
        
       float x_1 = cos(angle) * length_side_cube;
       float y_1 = sin(angle) * length_side_cube;
       float x = x_center_table + x_1 + 17.32;
       float y = y_center_table + y_1;
