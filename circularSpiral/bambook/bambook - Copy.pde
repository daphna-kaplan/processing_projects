...
void setup() {
 ...
 float factor = 1;
  for (int layer = 0; layer<50; layer++) {
   ...
   for (float angle = 0; angle<=TWO_PI; angle+=angle_increment) {
     ...
     factor -= 0.02;
  length_side_cube -= 0.2 * factor;
 }
 for (int layer = 51; layer<tot_layers; layer++) {
   current_z += layer_height;
   gCommand("G1 Z" + current_z);
   for (float angle = 0; angle<=TWO_PI; angle+=angle_increment) {
       float x_1 = cos(angle) * length_side_cube;
  ...
  for (float outerangle = 0; outerangle<=TWO_PI; outerangle+=TWO_PI /100.0f) {
   current_z += layer_height;
   gCommand("G1 Z" + current_z);
     for (int layer = 201; layer<600; layer++) {
  ...
  for (float outerangle = 0; outerangle<=TWO_PI; outerangle+=TWO_PI /100.0f) {
   current_z += layer_height;
   gCommand("G1 Z" + current_z);
     for (int layer = 601; layer<1000; layer++) {
 ...
 for (int layer = 1001; layer<1200; layer++) {
   current_z += layer_height;
   gCommand("G1 Z" + current_z);
   for (float angle = 0; angle<=TWO_PI; angle+=angle_increment) {
       float x_1 = cos(angle) * length_side_cube;
